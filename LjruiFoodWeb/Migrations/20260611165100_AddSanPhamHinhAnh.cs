using Microsoft.EntityFrameworkCore.Migrations;

#nullable disable

namespace LjruiFoodWeb.Migrations
{
    /// <inheritdoc />
    public partial class AddSanPhamHinhAnh : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "SanPhamHinhAnh",
                columns: table => new
                {
                    Id = table.Column<int>(type: "int", nullable: false)
                        .Annotation("SqlServer:Identity", "1, 1"),
                    DuongDan = table.Column<string>(type: "nvarchar(500)", maxLength: 500, nullable: false),
                    SanPhamMa = table.Column<int>(type: "int", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_SanPhamHinhAnh", x => x.Id);
                    table.ForeignKey(
                        name: "FK_SanPhamHinhAnh_SanPham_SanPhamMa",
                        column: x => x.SanPhamMa,
                        principalTable: "SanPham",
                        principalColumn: "Ma",
                        onDelete: ReferentialAction.Cascade);
                });

            migrationBuilder.CreateIndex(
                name: "IX_SanPhamHinhAnh_SanPhamMa",
                table: "SanPhamHinhAnh",
                column: "SanPhamMa");
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "SanPhamHinhAnh");
        }
    }
}
